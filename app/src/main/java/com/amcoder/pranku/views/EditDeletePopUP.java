...
public class EditDeletePopUP {
    ...
    private List<RowItem> createRowItems(Context context) {
        ...
        String edit = context.getResources().getString(R.string.edit);
        String delete = context.getResources().getString(R.string.delete);
        String[] desc = {edit, delete};
        rowItems.add(new RowItem(VectorDrawable.create(context, R.drawable.edit_icon_17x17), desc[0]));
        if (!mDisableDelete) {
            rowItems.add(new RowItem(VectorDrawable.create(context, R.drawable.trash_bin), desc[1]));
        }
        return rowItems;
    }
